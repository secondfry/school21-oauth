...
import { getServerSession, NEXT_AUTH_SESSION_TOKEN } from '$lib/endpoints/nextAuthSvelteHandler';
...
export const getNextAuthSession: Handle = async ({ event, resolve }) => {
  ...
  for (const { name, value, options } of cookies) {
    if (name === NEXT_AUTH_SESSION_TOKEN && event.locals.sessionTokenHasBeenSet) continue;
